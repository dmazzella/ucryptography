...
#define SWAP_B8_TO_B32(b32, b8, i)                                                                                                          \
    {                                                                                                                                       \
        (b32) = ((uint32_t)(b8)[(i) + 3]) | ((uint32_t)(b8)[(i) + 2] << 8) | ((uint32_t)(b8)[(i) + 1] << 16) | ((uint32_t)(b8)[(i)] << 24); \
    }
                       ...
                       size_t add_len)
{
    ...
    if (add_len != 0)
    {
        ...
        if (add_len % 4U != 0U)
        {
            size_t auth_pad_sz = ((add_len / 4) + 1) * 4;
            uint8_t *auth_in_padded = mbedtls_calloc(auth_pad_sz, sizeof(uint8_t));

            memset(auth_in_padded, 0, auth_pad_sz);
            memcpy(auth_in_padded, add, add_len);

            ctx->hcryp_gcm.Init.Header = (uint32_t *)auth_in_padded;
            /* header buffer in word */
            ctx->hcryp_gcm.Init.HeaderSize = (uint32_t)(auth_pad_sz / 4);
        }
        else
        {
