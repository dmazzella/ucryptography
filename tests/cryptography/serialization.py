...
RSA_ENCRYPTED_PRIVATE_KEY_PEM = b"""-----BEGIN ENCRYPTED PRIVATE KEY-----
...
def main():
    def ec_serialization():
        ...
        private_key = serialization.load_pem_private_key(EC_PRIVATE_KEY_PEM, None)
        ...
        public_key1 = serialization.load_pem_public_key(EC_PUBLIC_KEY_PEM)
    ...
    def rsa_serialization():
        private_key = serialization.load_pem_private_key(RSA_PRIVATE_KEY_PEM, None)
        ...
        public_key1 = serialization.load_pem_public_key(RSA_PUBLIC_KEY_PEM)
    ...
    def rsa_e_serialization():
        ...
        private_key = serialization.load_pem_private_key(
            RSA_ENCRYPTED_PRIVATE_KEY_PEM, b"password"
        )
        public_numbers = private_key.public_key().public_numbers()
        print("n", public_numbers.n)
        print("e", public_numbers.e)
    ...
    def encrypted_serialization():
        password = b"passphrase"

        def _roundtrip(key, label):
            enc = key.private_bytes(
                encoding=serialization.Encoding.PEM,
                format=serialization.PrivateFormat.TraditionalOpenSSL,
                encryption_algorithm=serialization.BestAvailableEncryption(password),
            )
            assert b"ENCRYPTED" in enc, label + " not encrypted"
            back = serialization.load_pem_private_key(enc, password)
            clear = serialization.PrivateFormat.TraditionalOpenSSL
            assert back.private_bytes(
                encoding=serialization.Encoding.DER,
                format=clear,
                encryption_algorithm=serialization.NoEncryption(),
            ) == key.private_bytes(
            ...
            wrong = False
            try:
                serialization.load_pem_private_key(enc, b"wrong")
            except Exception:
                wrong = True
            assert wrong, label + " wrong password did not raise"
            print("encrypted", label, "OK")

        _roundtrip(ec.generate_private_key(ec.SECP256R1()), "EC")
        _roundtrip(
